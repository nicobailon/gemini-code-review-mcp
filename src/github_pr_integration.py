...
def parse_github_pr_url(url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if path_parts[2] != 'pull':
            raise ValueError("Invalid GitHub PR URL: Must be a pull request URL (not issue or other)")
...
def validate_github_token(token: str, base_url: str = "https://api.github.com") -> bool:
    ...
    try:
        ...
        api_url = f"{base_url}/user" if 'api.' in base_url else f"{base_url}/api/v3/user"
...
def get_pr_file_changes(owner: str, repo: str, pr_number: int, token: str,
                       base_url: str = "https://api.github.com") -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if response.status_code != 200:
            raise ValueError(f"Failed to fetch PR file changes: {response.status_code} - {response.text}")
...
def get_complete_pr_analysis(pr_url: str, token: Optional[str] = None) -> Dict[str, Any]:
    """
