...
def _discover_nested_claude_files(project_path: str, discovered_files: List[Dict[str, Any]], visited_paths: set) -> None:
    """
...
def discover_user_level_claude_md(user_home_override: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Could not read user-level CLAUDE.md file {user_claude_file}: {e}")
            return None
...
def discover_enterprise_level_claude_md(enterprise_dir_override: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        for enterprise_dir in enterprise_directories:
            ...
            try:
                ...
                logger.warning(f"Could not read enterprise-level CLAUDE.md file {enterprise_claude_file}: {e}")
                continue
...
def discover_configuration_files(
    ...
    try:
        claude_files = discover_all_claude_md_files(project_path, user_home_override, enterprise_dir_override)
...
def discover_legacy_cursorrules(project_path: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Could not read legacy .cursorrules file {cursorrules_file}: {e}")
            return None
        ...
        for mdc_file in mdc_files:
            try:
                ...
                if not metadata or 'description' not in metadata:
                    logger.warning(f"Skipping malformed MDC file {mdc_file}: no description in frontmatter")
                    continue
