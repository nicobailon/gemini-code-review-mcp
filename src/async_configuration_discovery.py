...
async def async_read_files(file_paths: List[str], max_workers: int = 10) -> Dict[str, str]:
    """
...
async def async_discover_legacy_cursorrules(project_path: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(_run_async_discovery_in_new_loop, 
                                       project_path, include_claude_memory, include_cursor_rules)
                result = future.result(timeout=30)  # 30 second timeout
                logger.info(f"Async configuration discovery completed in {result['performance_stats']['discovery_time_ms']}ms")
                return result
        except RuntimeError:
        ...
        logger.warning(f"Async discovery failed ({type(async_error).__name__}: {async_error}), trying threaded approach")
        
        # Strategy 2: Try threaded synchronous discovery (medium speed)
        try:
            result = _threaded_sync_discovery(project_path, include_claude_memory, include_cursor_rules)
            end_time = time.time()
            result['performance_stats']['discovery_time_ms'] = int((end_time - start_time) * 1000)
            result['performance_stats']['fallback_method'] = 'threaded_sync'
            logger.info(f"Threaded configuration discovery completed in {result['performance_stats']['discovery_time_ms']}ms")
            return result
            
        except Exception as threaded_error:
            logger.warning(f"Threaded discovery failed ({type(threaded_error).__name__}: {threaded_error}), using basic fallback")
            
            # Strategy 3: Basic synchronous fallback (slowest but bulletproof)
            try:
                result = _basic_sync_discovery(project_path, include_claude_memory, include_cursor_rules)
                end_time = time.time()
                result['performance_stats']['discovery_time_ms'] = int((end_time - start_time) * 1000)
                result['performance_stats']['fallback_method'] = 'basic_sync'
                logger.info(f"Basic configuration discovery completed in {result['performance_stats']['discovery_time_ms']}ms")
                return result
...
def _run_async_discovery_in_new_loop(project_path: str, include_claude_memory: bool, include_cursor_rules: bool) -> Dict[str, Any]:
    """Run async discovery in a new event loop (for thread execution)."""
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        return loop.run_until_complete(
            async_discover_all_configurations(project_path, include_claude_memory, include_cursor_rules)
        )
    finally:
        loop.close()


def _threaded_sync_discovery(project_path: str, include_claude_memory: bool, include_cursor_rules: bool) -> Dict[str, Any]:
            ...
            try:
                ...
                if task_result:
                    if 'claude_memory' in str(task_result):
                        result['claude_memory_files'].extend(task_result.get('files', []))
                    elif 'cursor' in str(task_result):
                        result['cursor_rules'].extend(task_result.get('files', []))
            except Exception as e:
                logger.warning(f"Threaded task failed: {e}")
    
    result['performance_stats']['total_files_read'] = len(result['claude_memory_files']) + len(result['cursor_rules'])
    return result


def _basic_sync_discovery(project_path: str, include_claude_memory: bool, include_cursor_rules: bool) -> Dict[str, Any]:
