    ...
    try:
        ...
        if context_content is not None:
            ...
            if not content:
                raise ValueError("Context content cannot be empty")
            analyzed_length = len(content)
            project_for_config = os.path.dirname(context_file_path)  # Use parent directory for config discovery
            
        elif project_path is not None:
            # Generate context first, then analyze
            from generate_code_review_context import generate_code_review_context_main as generate_context
