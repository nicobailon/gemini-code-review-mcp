...
def analyze_project_for_meta_prompt(project_path: str, scope: str = "recent_phase") -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            project_data['file_structure_summary'] = _generate_lightweight_structure_summary(project_path)
        except Exception as e:
...
def _generate_lightweight_structure_summary(project_path: str) -> str:
    ...
    try:
        ...
        common_files = ['package.json', 'pyproject.toml', 'requirements.txt', 'Cargo.toml', 'go.mod', 'README.md', 'CLAUDE.md']
...
def _get_lightweight_git_context(project_path: str) -> str:
    ...
    try:
        ...
        return "Git context:\n" + "\n".join(git_info) if git_info else "Git repository detected"
        
    except Exception:
        return "No git context available"


def generate_meta_prompt_from_analysis(project_data: Dict[str, Any], 
    ...
    try:
        ...
        if custom_template:
            ...
            if env_template:
                ...
                if not template:
                    raise ValueError("Default meta-prompt template not found in configuration")
...
def generate_optimized_meta_prompt(project_path: str, 
    ...
    try:
        ...
        meta_prompt_result = generate_meta_prompt_from_analysis(project_data, custom_template, temperature)
