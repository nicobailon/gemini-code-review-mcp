            ...
            try:
                ...
                if os.path.isfile(resolved_path):
                    ...
                    if current_depth + 1 < max_depth:
                        ...
                        normalized_resolved = os.path.abspath(resolved_path)
                        ...
                        if nested_result['recursion_limit_hit']:
                            recursion_hit = True
                        max_depth_reached = max(max_depth_reached, nested_result['max_depth_reached'])
                        
                        # Add nested imports (avoid duplicates based on resolved path)
                        for nested_import in nested_result['all_imports']:
                            if not any(imp['resolved_path'] == nested_import['resolved_path'] for imp in all_imports):
    ...
    for import_path in parsed_file['imports']:
        try:
            ...
            try:
                ...
                if current_depth + 1 < max_depth and os.path.abspath(resolved_path) not in visited_files:
